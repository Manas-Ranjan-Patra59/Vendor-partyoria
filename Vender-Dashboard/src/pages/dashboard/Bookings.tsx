...
const Bookings = () => {
  ...
  // Get vendor's profession from onboarding data with fallback
  const [vendorData, setVendorData] = useState<any>({});
  const [vendorProfession, setVendorProfession] = useState<string>("Photography");
  ...
  useEffect(() => {
    ...
    const loadVendorData = () => {
      try {
        const storedVendorData = localStorage.getItem("vendorOnboarding");
        if (storedVendorData) {
          const parsedData = JSON.parse(storedVendorData);
          setVendorData(parsedData);
          setVendorProfession(parsedData.business || "Photography");
          console.log('Loaded vendor data:', parsedData);
        } else {
          console.log('No vendor data found, using defaults');
          setVendorData({ business: "Photography", services: [] });
          setVendorProfession("Photography");
        }
      } catch (error) {
    ...
    // Listen for verification status change
    const handleVerificationUpdate = () => {
      const newStatus = getVerificationStatus();
      const newProfile = JSON.parse(localStorage.getItem('vendor_profile') || '{}');
      const newVerified = newStatus === "approved" || newProfile.is_verified;
  ...
  useEffect(() => {
    if (isVerified && vendorProfession && vendorData.business) {
      fetchBookings();
    } else if (!isVerified) {
      setLoading(false);
    }
  }, [isVerified, vendorProfession, vendorData.business]);

  const fetchBookings = async () => {
    setLoading(true);
    
    try {
      console.log('Fetching bookings for profession:', vendorProfession);
      console.log('Vendor data:', vendorData);
      
      // Use mock data based on profession
      console.log('Vendor profession from state:', vendorProfession);
      console.log('Vendor data business:', vendorData.business);
      const allBookings = getBookingsByProfession(vendorProfession);
      console.log('All bookings for profession:', vendorProfession, ':', allBookings.length);
      console.log('Sample booking:', allBookings[0]);
      
      // Show all bookings for the vendor's profession
      console.log('Setting bookings:', allBookings.length);
      setBookings(allBookings);
      
      // Store bookings in localStorage for Profile page statistics
      localStorage.setItem('bookingsData', JSON.stringify(allBookings));
    } catch (error) {
      console.error('Error fetching bookings:', error);
      // Fallback to showing all bookings for the profession
      const allBookings = getBookingsByProfession(vendorProfession);
      setBookings(allBookings);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-4xl font-bold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">Bookings</h1>
        <Badge variant="secondary" className="px-4 py-2 text-lg font-semibold bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-800">
          {filteredBookings?.length || 0} bookings
        </Badge>
      </div>

      <Card className="bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 border-none shadow-xl">
          ...
          <div className="flex gap-4 mt-6">
            <div className="flex-1 relative">
              ...
              <Input
                ...
                className="pl-12 h-12 text-lg bg-white/70 backdrop-blur-sm border-white/20 focus:bg-white/90 transition-all"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="w-[200px] h-12 bg-white/70 backdrop-blur-sm border-white/20">
        ...
        <CardContent className="bg-white/60 backdrop-blur-sm rounded-b-lg">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-8 h-8 border-3 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
              <span className="ml-3 text-lg font-medium">Loading bookings...</span>
            </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 mb-4">
                {searchTerm || filterStatus !== "all" 
                  ...
                  : `No bookings found for ${vendorProfession}. Bookings will appear here once customers book your services.`
                }
              </p>
              {(!searchTerm && filterStatus === "all") && (
                <p className="text-sm text-gray-500">
                  Selected services: {vendorData.services?.join(", ") || "None"}
                </p>
              )}
            </div>
          ) : (
            <div className="overflow-hidden rounded-xl">
              <Table>
                <TableHeader>
                  <TableRow className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:from-indigo-700 hover:via-purple-700 hover:to-pink-700">
                    ...
                    <TableHead className="text-white font-semibold">Guests</TableHead>
                ...
                <TableBody>
                  {filteredBookings.map((booking, index) => (
                    <TableRow key={booking.id} className={`${index % 2 === 0 ? 'bg-white/80' : 'bg-indigo-50/50'} hover:bg-purple-100/70 transition-colors`}>
                      <TableCell className="font-semibold text-gray-800">{booking.customer}</TableCell>
                      <TableCell className="text-gray-700">{booking.service}</TableCell>
                      <TableCell className="text-gray-700">{booking.date}</TableCell>
                      <TableCell className="text-gray-700">{booking.guests || 'N/A'}</TableCell>
                      <TableCell className="font-semibold text-green-600">{booking.amount}</TableCell>
                      <TableCell>
                        <Badge className={`${getStatusColor(booking.status)} hover:!bg-transparent hover:!text-inherit pointer-events-none font-medium`}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            size="sm"
                            variant="outline"
                            className="hover:bg-purple-50 hover:border-purple-300 hover:text-purple-600 transition-all"
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-3xl bg-gradient-to-br from-white via-indigo-50 to-purple-50 border-none shadow-2xl">
          <DialogHeader className="pb-6">
            <DialogTitle className="text-2xl font-bold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent flex items-center gap-2">
              <span className="text-2xl">📋</span> Booking Details
            </DialogTitle>
          </DialogHeader>
          {selectedBooking && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">👤</span> Customer
                  </h3>
                  <p className="text-lg font-semibold text-gray-800">{selectedBooking.customer}</p>
                </div>
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">📊</span> Status
                  </h3>
                  <Badge className={`${getStatusColor(selectedBooking.status)} hover:!bg-transparent hover:!text-inherit pointer-events-none font-medium text-sm px-3 py-1`}>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  ...
                  <p className="text-lg font-medium text-gray-800">{selectedBooking.service}</p>
                </div>
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">💰</span> Amount
                  </h3>
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">👥</span> Guests
                  </h3>
                  <p className="text-lg font-semibold text-gray-800">{selectedBooking.guests || 'N/A'}</p>
                </div>
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">📅</span> Date
                  </h3>
                  <p className="text-lg font-medium text-gray-800">{selectedBooking.date}</p>
                </div>
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">⏰</span> Time
                  </h3>
                  <p className="text-lg font-medium text-gray-800">{selectedBooking.time}</p>
                </div>
              </div>
              ...
              <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                ...
                <p className="text-lg font-medium text-gray-800">{selectedBooking.location}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  ...
                  <p className="text-lg font-medium text-gray-800">{selectedBooking.phone}</p>
                </div>
                <div className="p-4 bg-white/70 backdrop-blur-sm rounded-xl border border-white/20">
                  <h3 className="font-semibold text-sm text-indigo-600 mb-2 flex items-center gap-1">
                    <span className="text-base">📧</span> Email
                  </h3>
                  <p className="text-lg font-medium text-gray-800">{selectedBooking.email}</p>
                </div>
              </div>
            </div>
          )}
